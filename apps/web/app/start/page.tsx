...
const STEPS = [
  {
    ...
    fields: [
      ...
      {
        key: "offerType", label: "Offer Type", required: true, type: "select",
        options: ["coaching", "consulting", "done-for-you", "course", "hybrid"],
      },
      {
        key: "deliveryFormat", label: "Delivery Format", required: true, type: "select",
        options: ["1:1", "group", "async", "mixed"],
      },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "buyerAuthority", label: "Purchasing Authority", required: true, type: "select",
        options: ["self", "manager", "owner", "committee"],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "proofAssets", label: "Proof Assets", placeholder: "e.g. 3 case studies with revenue results, 12 testimonials, featured in Forbes, 5 years as VP Sales...", required: false, type: "textarea", maxLen: 1200 },
    ],
  },
  {
    id: 5,
...
export default function StartPage() {
  ...
  // Initialize Turnstile when on last step
  const initTurnstile = () => {
      ...
      <div className="pt-24 pb-16 px-6 max-w-2xl mx-auto">
        ...
        <div className="mb-12">
          <div className="flex items-center justify-between mb-3">
            <p className="mono text-xs text-parchment-muted tracking-wider">
            ...
            {STEPS.map((s) => (
              <div
                ...
                className={`h-1.5 w-1.5 rounded-full transition-colors ${
                  s.id < step ? "bg-gold" : s.id === step ? "bg-gold-light" : "bg-[#2a2a2a]"
                }`}
              />
            ))}
          </div>
        ...
        <div key={step} className="fade-up">
          ...
          <div className="space-y-8">
                  ...
                  <div>
                    ...
                    {(field as any).maxLen && (
                      <p className="mono text-xs text-parchment-muted mt-1 text-right">
                        {(data[field.key] || "").length}/{(field as any).maxLen}
                      </p>
                    )}
                  </div>
                ) : (
            ...
            {/* Consent + Turnstile on last step */}
            {currentStep.isLast && (
              <>
                <div>
                  <label className="flex items-start gap-3 cursor-pointer group">
                    <div
                      className={`mt-0.5 w-4 h-4 border shrink-0 flex items-center justify-center transition-colors ${
                        consent ? "bg-gold border-gold" : "border-[#3a3a3a] group-hover:border-gold"
                      }`}
                ...
                <div>
                  ...
                  {!turnstileToken && (
                    <p className="text-xs text-parchment-muted mt-2">
                      Complete the verification above before proceeding.
                    </p>
                  )}
                </div>
              </>
          ...
          <div className="mt-12 flex items-center justify-between">
            ...
            {step < totalSteps ? (
              <button
                onClick={handleNext}
                className="bg-gold text-ink px-8 py-3 text-sm tracking-widest uppercase hover:bg-gold-light transition-colors"
              >
                Continue →
              </button>
            ) : (
